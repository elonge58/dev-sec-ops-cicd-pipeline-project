...
pipeline{
    ...
    stages {
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'DockerHub-Credential', toolName: 'Docker'){
                       ...
                       sh "docker tag reddit jude58/reddit:latest "
                       sh "docker push jude58/reddit:latest "
                    }
                }
            }
        }
        stage("Trivy App Image Scan"){
            steps{
                sh "trivy image jude58/reddit:latest > trivy_image_analysis_report.txt"
            }
        }
        ...
        stage('ZAP Dynamic Testing | DAST') {
            steps {
                sshagent(['OWASP-Zap-Credential']) {
                    sh 'ssh -o StrictHostKeyChecking=no ubuntu@3.147.82.42 "docker run -t owasp/zap2docker-stable zap-baseline.py -t http://3.135.197.12:30000/" || true'
    ...
    always {
        ...
        slackSend channel: '#jude58-devsecops-cicd-alerts', //update and provide your channel name
